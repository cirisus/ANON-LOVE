...
@keyframes blurThin {from {filter: blur(0);} to {filter: blur(6px);}}
    ...
    .progress-bar-container {
       ...
       > .progress-bar {
            ...
            animation: blurThin .3s ease 2s 1 forwards;
            &[data-pos="mid"] {
                width: 100%;
                box-shadow: none;
    ...
    .anon-sprite {
        ...
        background-size: contain;
        animation: leftInSprite .4s cubic-bezier(.4,0,.6,1) 2s 1 forwards, fade .4s cubic-bezier(.4,0,.6,1) 2.5s 1 reverse forwards, blurThin .4s cubic-bezier(.4,0,.6,1) 2.25s 1 reverse forwards;
    }
</style>