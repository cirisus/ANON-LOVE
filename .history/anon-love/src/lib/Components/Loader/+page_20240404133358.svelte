...
@keyframes leftIn {from {transform: translate(-50%, -50%);} to {transform: translate(-175%,-50%);}}
@keyframes fade {from {opacity: 1;} to {opacity: 0;}}
@keyframes blur {from {filter: blur(1rem);} to {filter: blur(0);}}
    #loader, #open-effects {
        position: fixed;
        left: 0;
        top: 0;
    ...
    .progress-bar-container {
        ...
        animation: cutProgressBar .3s cubic-bezier(.4,0,.6,1) 2.5s 1 forwards;
       > .progress-bar {
            height: 100%;
            position: relative;
