...
export function destroyLoader() {
    ...
    if (loader) {
        ...
        Promise.all(allAnimations.map(animation => animation.finished)).then(() => {
            ...
            if (tipOrigin) {
                tipOrigin.style.clipPath = `polygon(calc(-70 / sqrt(3))% 0%,0 0,calc(-70 / sqrt(3))% 70%,calc(-70 / sqrt(3))% 70%)`;
            }
            if (tipAlt) {
                tipAlt.style.clipPath = `polygon(calc(-70 / sqrt(3))% 0%,calc(100 + 70 / sqrt(3))% 0%,100% 70%,calc(-70 / sqrt(3))% 70%)`;
            }
            if (sweepLine) {
