<script>
    ...
    import { destroyLoader } from './scripts/loader.js';
    import { loadAnimation } from './scripts/lottie.js';
    let progress = 0;
    let isLoading = true;

    function smeltingOut(_node, { duration }) {
        return {
            duration,
            css: t => `
                opacity: ${1 - t};
                backdrop-filter: blur(${(1-t)*10}px);
                -webkit-backdrop-filter: blur(${(1-t)*10}px);
            `
        };
    }
...
{#if isLoading}
<div id="loader" outro:fade="{smeltingOut({duration: 1500})}">
