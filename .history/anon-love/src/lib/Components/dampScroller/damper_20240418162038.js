export default function handleScroll(scroller, scrollBoxes) {
    ...
    function checkViewport(child, index) {
        child.dataset.toggle = index === currentIndex ? 'show' : 'hide';

        // Check if the child is in viewport
    ...
    scrollBoxes.forEach((box, index) => {
        ...
        children.forEach(child => checkViewport(child, index));
    });
